...
const SummarizeLegalDocumentOutputSchema = z.object({
  ...
  keyPoints: z.array(z.string()).describe('Key points extracted from the legal document.'),
  suggestedSteps: z
    .array(z.string())
    .describe(
  ...
  })).describe('An analysis of potential risks found within the document, including suggestions for mitigation.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert legal analyst specializing in Indian Law. You will analyze the provided legal document, which can be in English or Marathi. Provide a summary and key points in English.

In addition to the summary and key points, analyze the document and perform the following tasks:
